...
function CharLoader(props) {
   ...
   const { setRace, setCharClass, setStats, setStStats, setSkills, setLevel,
      setProficiency, setLanguages, setWeapons, setEquip, setTraits, setIdeals, setFlaws, setBonds, setFeatures } = useContext(CharCreationContext);
   useEffect(() => {
      if (props.character !== null) {
         ...
         setTraits(props.character.traits);
         setIdeals(props.character.ideals);
         setFlaws(props.character.flaws);
         setBonds(props.character.bonds);
         setFeatures(props.character.features);
